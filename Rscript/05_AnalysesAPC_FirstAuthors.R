...
bootstrap_results_countries <- bind_rows(
  SubsampledPW.results_Solo_All[2],
...
source("./Rscript/functions/DivRichCalcTable_Solo.R")
Table2_Solo <- DivRichCalcTable_Solo(
  sole_author_pubs_ALL_first_author,
  sole_author_pubsNOCHNUSA_first_author,
...
source("./Rscript/functions/DivRichCalcSummaryTable_sampled.R")
Table2_CoAuthored <- DivRichCalcSummaryTable_sampled(
...
bootstrapped_PW_combined <- bind_rows(
  SubsampledPW.results_Solo_All[1],
  SubsampledPW.results_Solo_NoUSACHN[1],
  SubsampledPW.results_First_Co_All[1],
  SubsampledPW.results_First_Co_NOUSACHN[1]
...
RichnessOA <- c(
  as.numeric(Table2$OA_AllCountries[1:2]), as.numeric(Table2$OA_noCHNorUSA[1:2]),
  rep(NA, 4)
)
InvSimpOA <- c(
  as.numeric(Table2$OA_AllCountries[3:4]), as.numeric(Table2$OA_noCHNorUSA[3:4]),
  rep(NA, 4)
)
...
############################################################
AvgNumbAuthors <- AllData %>%
  # average number of authors per journal
  # filter(Year==2019) %>%
...
############################################################
AvgNumbAuthorsAll <- coauthor_pubs_ALL %>%
  # average number of authors per journal
  group_by(DOI) %>%
  filter(AuthorNum == max(AuthorNum)) %>%
  ungroup()
