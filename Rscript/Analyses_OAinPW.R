...
OAinPW_sole_all<-sole_author_pubs_ALL_first_author %>% filter(JrnlType=="OAinPW")
OAinPW_sole_noCHNUSA<-sole_author_pubsNOCHNUSA_first_author%>% filter(JrnlType=="OAinPW")

OAinPW_coauthored_all<-coauthor_pubs_ALL_first_author%>% filter(JrnlType=="OAinPW")
OAinPW_coauthored_noCHNUSA<-coauthor_pubsNOCHNUSA_first_author%>% filter(JrnlType=="OAinPW")


source("./Rscript/functions/DivRichCalc.R") 
OAinPW_data<-DivRichCalc(OAinPW_coauthored_all,"author_first","OAinPW")
OAinPW_data[1]
OAinPW_data[2]
OAinPW_data_countries<-as_tibble(OAinPW_data[3])

OAinPW_1stcountries<-ungroup(coauthor_pubs_ALL_first_author) %>% 
  filter(JrnlType=="OAinPW") %>% 
...
OAinPW_1stRegions<-ungroup(coauthor_pubs_ALL_first_author) %>% 
  filter(JrnlType=="OAinPW") %>% 
  select(Region) %>% 
  drop_na() %>% 
  group_by(Region) %>% 
...
OAinPW_1stIncome<-ungroup(coauthor_pubs_ALL_first_author) %>% 
  filter(JrnlType=="OAinPW") %>% 
  select(IncomeGroup) %>% 
  drop_na() %>% 
...
n_journalsPW<-n_journals[3,2]
n_journalsOAinPW<-n_journals[2,2]
############################################################
# APCs
############################################################
...
coauthor_pubs_ALL_first_author2<-coauthor_pubs_ALL_first_author %>% filter(JrnlType!="PW")
coauthor_pubs_ALL_first_author2$JrnlType<-gsub("OAinPW","PW",coauthor_pubs_ALL_first_author2$JrnlType)
as.factor(coauthor_pubs_ALL_first_author2$JrnlType)
SubsampledPW.results_First_Co_All2<-SubSamplePWvsOA_comparison(coauthor_pubs_ALL_first_author2,"author_first")

# Save df of Div and Rich results 
write.csv(SubsampledPW.results_First_Co_All2[1], 
          'output/SubsampledPW.results_RichDiv_CO_ALL2.csv', 
          row.names = FALSE)
write.csv(SubsampledPW.results_First_Co_All2[2], 
          'output/SubsampledPW.results_Countries_CO_ALL2.csv', 
          row.names = FALSE)
...
sole_author_pubs_ALL_first_author2<-sole_author_pubs_ALL_first_author %>% filter(JrnlType!="PW")
sole_author_pubs_ALL_first_author2$JrnlType<-gsub("OAinPW","PW",sole_author_pubs_ALL_first_author2$JrnlType)
as.factor(sole_author_pubs_ALL_first_author2$JrnlType)

source("./Rscript/functions/SubSamplePWvsOA_comparison.R")
SubsampledPW.results_Solo_All2<-SubSamplePWvsOA_comparison(sole_author_pubs_ALL_first_author2,"author_first")

# Save df of Div and Rich results
write.csv(SubsampledPW.results_Solo_All2[1],
          'output/SubsampledPW.results_RichDiv_SOLO_ALL2.csv',
          row.names = FALSE)
write.csv(SubsampledPW.results_Solo_All[2],
          'output/SubsampledPW.results_Countries_SOLO_ALL2.csv',
          row.names = FALSE)
...
sole_author_pubsNOCHNUSA_first_author2<-sole_author_pubsNOCHNUSA_first_author %>% filter(JrnlType!="PW")
sole_author_pubsNOCHNUSA_first_author2$JrnlType<-gsub("OAinPW","PW",sole_author_pubsNOCHNUSA_first_author2$JrnlType)
as.factor(sole_author_pubsNOCHNUSA_first_author2$JrnlType)

source("./Rscript/functions/SubSamplePWvsOA_comparison.R")
SubsampledPW.results_Solo_NoUSACHN2<-SubSamplePWvsOA_comparison(sole_author_pubsNOCHNUSA_first_author2,"author_first")
...
coauthor_pubsNOCHNUSA_first_author2<-coauthor_pubsNOCHNUSA_first_author %>% filter(JrnlType!="PW")
coauthor_pubsNOCHNUSA_first_author2$JrnlType<-gsub("OAinPW","PW",coauthor_pubsNOCHNUSA_first_author2$JrnlType)
as.factor(coauthor_pubsNOCHNUSA_first_author2$JrnlType)
source("./Rscript/functions/SubSamplePWvsOA_comparison.R")
SubsampledPW.results_First_Co_NOUSACHN2<-SubSamplePWvsOA_comparison(coauthor_pubsNOCHNUSA_first_author2,"author_first")

# Save df of Div and Rich results 
write.csv(SubsampledPW.results_First_Co_NOUSACHN2[1], 
          'output/SubsampledPW.results_RichDiv_CO_NOUSACHN2.csv', 
          row.names = FALSE)
write.csv(SubsampledPW.results_First_Co_NOUSACHN2[2], 
          'output/SubsampledPW.results_Countries_CO_NOUSACHN2.csv', 
          row.names = FALSE)
...
bootstrap_results_countries2<-bind_rows(SubsampledPW.results_Solo_All2[2],
                                       SubsampledPW.results_Solo_NoUSACHN2[2],
                                       SubsampledPW.results_First_Co_All2[2],
                                       SubsampledPW.results_First_Co_NOUSACHN2[2])




summary(as.factor(bootstrap_results_countries2$Dataset))

bootstrap_results_countries2<-bootstrap_results_countries2 %>% 
  mutate(author = ifelse(Dataset == "sole_author_pubs_ALL_first_author2", "solo", author)) %>% 
  mutate(author = ifelse(Dataset == "sole_author_pubsNOCHNUSA_first_author2", "solo", author)) %>% 
  mutate(Dataset = ifelse(Dataset == "sole_author_pubsNOCHNUSA_first_author2", "CHN & USA excluded", Dataset)) %>%
  mutate(Dataset = ifelse(Dataset == "coauthor_pubsNOCHNUSA_first_author2", "CHN & USA excluded", Dataset)) %>% 
  mutate(Dataset = ifelse(Dataset == "coauthor_pubs_ALL_first_author2", "All Countries", Dataset)) %>% 
  mutate(Dataset = ifelse(Dataset == "sole_author_pubs_ALL_first_author2", "All Countries", Dataset))

summary(bootstrap_results_countries2)
write.csv(bootstrap_results_countries2, "./output/bootstrap_results_countries2.csv", row.names = FALSE)
...
source("./Rscript/functions/DivRichCalcTable_Solo.R")
Table2_Solo2<-DivRichCalcTable_Solo(sole_author_pubs_ALL_first_author2,
                                   sole_author_pubsNOCHNUSA_first_author2,
                                   SubsampledPW.results_Solo_All2[1],
                                   SubsampledPW.results_Solo_NoUSACHN2[1])
Table2_Solo2
...
source("./Rscript/functions/DivRichCalcSummaryTable_sampled.R")
Table2_CoAuthored2<-DivRichCalcSummaryTable_sampled(coauthor_pubs_ALL_first_author2,
                                                   coauthor_pubsNOCHNUSA_first_author2,
                                                   SubsampledPW.results_First_Co_All2[1],
                                                   SubsampledPW.results_First_Co_NOUSACHN2[1])


Table2_CoAuthored2

Table2_Joint2<-bind_rows(Table2_CoAuthored2,Table2_Solo2)
write.csv(Table2_Joint2, "./tables_figs/Table22.csv", row.names = FALSE)
# Table2_Joint<-read_csv("./tables_figs/Table2.csv")

# Table 2 
source("./Rscript/functions_figures/Table2.R")
Table22<-Table2(sole_author_pubs_ALL_first_author2,
               sole_author_pubsNOCHNUSA_first_author2,
               coauthor_pubs_ALL_first_author2,
               coauthor_pubsNOCHNUSA_first_author2)
Table22
names(Table22)<-c("Metric","Author","OA","Mean PW",
                 "PW 95% CI", "OA",
                 "Mean PW","PW 95% CI")
Table22
# ##################################################
...
bootstrapped_PW_combined2<-bind_rows(SubsampledPW.results_Solo_All2[1],
                                    SubsampledPW.results_Solo_NoUSACHN2[1],
                                    SubsampledPW.results_First_Co_All2[1],
                                    SubsampledPW.results_First_Co_NOUSACHN2[1])
...
bootstrapped_PW_combined2$JrnlType<-"PW"
str(bootstrapped_PW_combined2)
...
summary(bootstrapped_PW_combined2)


summary(as.factor(bootstrapped_PW_combined2$Dataset))
bootstrapped_PW_combined2<-bootstrapped_PW_combined2 %>% 
  mutate(author = ifelse(Dataset == "sole_author_pubs_ALL_first_author2", "solo", author)) %>% 
  mutate(author = ifelse(Dataset == "sole_author_pubsNOCHNUSA_first_author2", "solo", author)) %>% 
  mutate(Dataset = ifelse(Dataset == "sole_author_pubsNOCHNUSA_first_author2", "CHN & USA excluded", Dataset)) %>%
  mutate(Dataset = ifelse(Dataset == "coauthor_pubsNOCHNUSA_first_author2", "CHN & USA excluded", Dataset)) %>% 
  ...
  mutate(Dataset = ifelse(Dataset == "sole_author_pubs_ALL_first_author2", "All Countries", Dataset))
summary(as.factor(bootstrapped_PW_combined2$Dataset))
bootstrapped_PW_combined<2-as.data.frame(bootstrapped_PW_combined2)

bootstrap_results2<-bootstrapped_PW_combined2
str(bootstrapped_PW_combined2)
bootstrap_results2$author<-as.factor(bootstrap_results2$author)
bootstrap_results2$author<-ordered(bootstrap_results2$author, 
                                  levels = c("solo", 
                                             "author_first"))
levels(as.factor(bootstrapped_PW_combined2$author))
levels(as.factor(bootstrapped_PW_combined2$Dataset))
