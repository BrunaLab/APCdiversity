...
countries<-vars[4]
  ...
  Subsampled_Income_summary_plot$author<-ordered(Subsampled_Income_summary_plot$author, levels = c("solo", "author_first", "author_last","author_all"))
  ...
  Fig1<-ggplot(fig_data,aes(x=perc,fill=IncomeGroup)) +
    ...
    scale_x_continuous(breaks = seq(0,100, by=10),expand = c(0.1, 0))+
